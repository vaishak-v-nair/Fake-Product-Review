...
const DemoSection = () => {
  ...
  const analyzeReview = async () => {
    ...
    // Mock analysis logic based on text patterns
    const text = reviewText.toLowerCase();
    const hasExcessivePunctuation = (reviewText.match(/[!?]{2,}/g) || []).length > 2;
    const hasAllCaps = (reviewText.match(/[A-Z]{5,}/g) || []).length > 2;
    const hasSuspiciousPhrases =
      text.includes("best ever") ||
      text.includes("buy now") ||
      text.includes("don't wait");
    const hasNuancedContent =
      text.includes("though") ||
      text.includes("however") ||
      text.includes("but") ||
      text.includes("could be");
    ...
    if (hasExcessivePunctuation || hasAllCaps) {
      verdict = "fake";
      confidence = 94;
      signals = [
        ...
        "Language patterns match known fake reviews",
      ];
    } else if (hasSuspiciousPhrases && !hasNuancedContent) {
      verdict = "suspicious";
      confidence = 72;
      signals = [
        "Contains promotional language",
        "Lacks balanced perspective",
        "Missing specific product details",
      ];
    } else if (hasNuancedContent || hasSpecificDetails) {
      verdict = "authentic";
      confidence = 88;
      signals = [
        "Contains balanced pros and cons",
        "Specific usage details present",
    ...
    setResult({ verdict, confidence, signals });
